"""FastAPI server — Human-in-the-Loop Approval System for AgentIQ.

New endpoints for human-in-the-loop workflow:
  POST /projects/{pid}/run?mode=human_in_loop  # Start with human approval
...
from fastapi import FastAPI, File, HTTPException, UploadFile, Query, BackgroundTasks
...
app = FastAPI(title="AgentIQ Human-in-Loop API", version="4.0.0")
...
def get_approval_status(project_id: str):
    ...
    for agent_num, agent_status in approvals.items():
        if "timestamp" in agent_status:
            try:
                start_time = datetime.fromisoformat(agent_status["timestamp"])
                elapsed = (datetime.now() - start_time).total_seconds()
...
# ── Individual agent endpoints (for backward compatibility) ───────────────────
...
# Similar endpoints for agents 2-6 would be here...
